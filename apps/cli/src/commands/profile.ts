...
export async function profile(username: string, options: { json?: boolean; quests?: boolean }) {
  if (!options.json) {
    console.log(chalk.blue(`Fetching RuneMetrics profile for: ${username}...`));
  }
  
  const data = await getRSProfile(username);

  if (!data) {
    if (options.json) {
      console.log(JSON.stringify({ error: "Profile not found or private" }, null, 2));
    } else {
  ...
  console.log(`${chalk.bold("Quests:")} ${data.questsComplete} complete, ${data.questsStarted} started, ${data.questsNotStarted} not started`);

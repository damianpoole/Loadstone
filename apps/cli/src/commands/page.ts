...
export async function page(title: string, options: { json?: boolean; section?: string }) {
  if (!options.json) {
  ...
  const data = await getPage(title, { json: true, section: options.section }) as any;
  ...
  if (options.json) {
    ...
    if (options.section) {
      ...
      const key = Object.keys(structuredContent).find(k => k.toLowerCase().includes(options.section!.toLowerCase()));
      if (key) {
  ...
  if (options.section) {
    ...
    if (key) {
      ...
      console.log(chalk.dim(`Available sections: ${Object.keys(sections).join(", ")}`));
    }
    return;
  }
